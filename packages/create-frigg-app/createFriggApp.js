...
function init() {
    const program = new commander.Command(packageJson.name)
        ...
        .on('--help', () => {
            ...
            console.log(
                `      ${chalk.cyan(
                    'https://github.com/friggframework/create-frigg-app/issues/new'
                )}`
            );
            console.log();
        })
            ...
            .run(
                {
                    ...
                    npmPackages: [],
                    npmGlobalPackages: ['create-frigg-app'],
                },
                {
                    duplicates: true,
                    showNotFound: true,
    ...
    checkForLatestVersion()
        .catch(() => {
            try {
                return execSync('npm view create-frigg-app version').toString().trim();
            } catch (e) {
                return null;
            }
        })
        .then(latest => {
            if (latest && semver.lt(packageJson.version, latest)) {
                ...
                console.error(
                    chalk.yellow(
                        `You are running \`create-frigg-app\` ${packageJson.version}, which is behind the latest release (${latest}).\n\n` +
                        'We recommend always using the latest version of create-frigg-app if possible.'
                    )
                );
                console.log();
                console.log(
                    'The latest instructions for creating a new app can be found here:\n' +
                    'https://docs.friggframework.org/getting-started/'
                );
                console.log();
            } else {
...
function getTemplateInstallPackage(template, originalDirectory) {
    let templateToInstall = 'cfa-template';
    if (template) {
        if (template.match(/^file:/)) {
            ...
            // Add prefix 'cfa-template-' to non-prefixed templates, leaving any
            ...
            if (
                ...
                // - @SCOPE/cfa-template
                // - cfa-template-NAME
                // - @SCOPE/cfa-template-NAME
                templateToInstall = `${scope}${templateName}${version}`;
            } else if (version && !scope && !templateName) {
                // Covers using @SCOPE only
                templateToInstall = `${version}/${templateToInstall}`;
            } else {
                // Covers templates without the `cfa-template` prefix:
                // - NAME
                // - @SCOPE/NAME
    ...
    if (!semver.satisfies(process.version, packageJson.engines.node)) {
        console.error(
            chalk.red(
                ...
                'Create Frigg App requires Node %s or higher. \n' +
...
function checkForLatestVersion() {
    return new Promise((resolve, reject) => {
        https
            .get(
                'https://registry.npmjs.org/-/package/create-frigg-app/dist-tags',
                res => {
